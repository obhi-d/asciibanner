...
class FigletFont(object):
    ...
    def preloadFont(cls, font):
        ...
        for extension in ('tlf', 'flf'):
            ...
            fn = os.path.dirname(os.path.realpath(__file__)) + os.path.sep + "fonts" + os.path.sep + fn
            if os.path.isfile(fn):
              with open(fn, "rb") as res:
    ...
    def getFonts(cls):
        return [font.rsplit('.', 2)[0] for font
                in os.path.dirname(os.path.realpath(__file__)) + os.path.sep + "fonts"
                if font.endswith(('.flf', '.tlf')) and cls.reMagicNumber.search(pkg_resources.resource_stream('pyfiglet.fonts', font).readline().decode('ascii', 'replace'))]
    ...
    def infoFont(cls, font, short=False):
        ...
        for line in  data.splitlines()[0:100]:
            if reMagicNumber.search(line) is None and reStartMarker.search(line) is None and reEndMarker.search(line) is None:
                infos.append(line)
